...
class IBHistoricalDownloader(EWrapper, EClient):
    def __init__(self, symbols: List[str], year: int, granularity: str, output_dir: str, resume: bool = False):
        ...
        self.original_symbols = symbols.copy()
    ...
    def filter_symbols_for_resume(self) -> Tuple[List[str], List[str]]:
        ...
        self.logger.info(f"Resume check for year {self.year}: {len(already_downloaded)} symbols already downloaded, {len(symbols_to_download)} remaining")
    ...
    def nextValidId(self, orderId: int):
        ...
        if self.resume and not self.resume_checked:
            ...
            if not symbols_to_download:
                self.logger.info(f"All symbols for year {self.year} already downloaded. Moving to next year...")
    ...
    def download_next_symbol(self):
        ...
        if not self.symbols:
            self.logger.info(f"Year {self.year} complete. Processed {len(self.completed_symbols)} symbols successfully.")
            self.total_symbols_successful += len(self.completed_symbols)
            self.total_symbols_processed += (len(self.completed_symbols) + len(self.failed_symbols) +
                                            len(self.invalid_symbols) + len(self.no_data_symbols) +
                                            len(self.timeout_symbols))
    ...
    def print_download_summary(self):
        """Print detailed summary of download results for current year"""
        self.logger.info("\n" + "="*70)
        self.logger.info(f"YEAR {self.year} DOWNLOAD SUMMARY")
        self.logger.info("="*70)
        self.logger.info(f"Granularity: {self.granularity}")
        self.logger.info(f"Total symbols for year: {len(self.original_symbols)}")
        ...
        if total_possible > 0:
            ...
            self.logger.info(f"\nYear {self.year} progress: {total_downloaded}/{total_possible} ({pct:.1f}%)")
...
def parse_arguments():
    ...
    symbol_group.add_argument(
        '--tickers-file',
        type=str,
        help='Path to tickers.csv file (can be year-specific or combined)'
    )
...
def main():
    ...
    if args.symbols:
        ...
        print(f"Using {len(symbols)} symbols from command line (will use same list for all years)")
    elif args.tickers_file:
        symbols = load_tickers_from_file(args.tickers_file)
        if not symbols:
            print(f"Error: No symbols found in {args.tickers_file}")
            sys.exit(1)
    else:
        print("Error: Either --symbols or --tickers-file must be provided")
        sys.exit(1)
    ...
    print(f"\nStarting download for {len(symbols)} symbols across {len(years)} years...")
    ...
    for year_idx, year in enumerate(years, 1):
        ...
        print(f"Estimated maximum time: {total_timeout} seconds")
        ...
        if app.isConnected():
            elapsed = time.time() - start_time
            app.logger.info(f"Year {year} processing complete after {elapsed:.1f} seconds. Disconnecting...")
            app.disconnect()
        ...
        # Update totals
        total_years_completed += 1
        total_symbols_downloaded += len(app.completed_symbols)
        total_symbols_attempted += len(app.original_symbols)

    # Final summary across all years
    print("\n" + "="*80)
    print("FINAL SUMMARY - ALL YEARS COMPLETE")
    print("="*80)
