...
class DownloadApp(EClient, ibapi.wrapper.EWrapper):
    ...
    def save_data(self, contract: Contract, bars: BarDataList) -> None:
        ...
        symbol = make_download_path(self.args, contract).split('/')[-1]
        if self.daily_files:
            # just overwrite whatever is there
            path = "%s.csv" % make_download_path(self.args, contract)
            df.to_csv(path, index=False)
        elif self.monthly_files:
            # depending on how things moved along, we'll have data
            # from different dates.

            for ym in df['date'].dt.strftime("%Y-%m").unique():
                #for m in df['date'].dt.month.unique():
                    path = os.path.sep.join(
                            [make_download_path(self.args, contract), f"{symbol}_{ym}.csv",]
                    )
                    new_bars = df.loc[(df['date'].dt.year == ym.strip('-')[0]) & (df['date'].dt.month == ym.strip('-')[1])]
        ...
        elif self.yearly_files:
            # depending on how things moved along, we'll have data
            # from different dates.

            for y in df['date'].dt.year.unique():
                path = os.path.sep.join(
                        [make_download_path(self.args, contract), f"{symbol}_{y}.csv",]
                )
                new_bars = df.loc[df['date'].dt.year == y]
                # if a file exists, let's attempt to load it, merge our data in, and then save it
                if os.path.exists(path):
                    existing_bars = pd.read_csv(path, parse_dates=['date'])
                    combined = pd.concat([existing_bars, new_bars])
