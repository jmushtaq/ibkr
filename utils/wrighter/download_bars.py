...
class DownloadApp(EClient, ibapi.wrapper.EWrapper):
    def __init__(self, contracts: ContractList, args: argparse.Namespace):
        ...
        #self.error_log = f'/var/www/opstrat/{args.base_directory}/logs/stock_errors_{datetime.now().strftime("%y%m%dT%H%M%S")}.log'
    ...
    def historicalDataRequest(self, contract: Contract) -> None:
        try:
            ...
            self.reqHistoricalData(
                ...
                #self.current.strftime(f"%Y%m%d-%H:%M:%S US/Eastern"),
                self.current.strftime(f"%Y%m%d-%H:%M:%S"),
    ...
    def save_data(self, contract: Contract, bars: BarDataList) -> None:
        ...
        #import ipdb; ipdb.set_trace()
        #if self.daily_files() and not df.empty:
        if self.daily_files():
            # just overwrite whatever is there
            #path = "%s.csv" % make_download_path(self.args, contract)
            #path = df['date'].dt.date.
            df = df[df.index.year==self.args.start_date.year]
            path = make_download_path(self.args, contract)
            path = f'{path}/SPY_{self.args.start_date.year}'

            if not os.path.exists(path):
               os.makedirs(path)

            file = f"{path}/{contract.symbol}_{self.args.size.replace(' ', '')}_{self.args.start_date.year}.csv"
    ...
    def handle_end(self, reqId: int, start: str) -> None:
        try:
            ...
            with open(self.args.success_log, 'a') as f1:
                f1.write(f'{self.args.symbol[0]}\n')

        except Exception as e:
            with open(self.args.error_log, 'a') as f2:
                f2.write(f'{self.args.symbol[0]}\n')
    ...
    def error(self, req_id: TickerId, error_code: int, error: str, advancedOrderRejectJson: str=""):
        ...
        if req_id < 0:
            ...
            sleep(3) # IBKR throttling issue, I think!
...
def main():
    ...
    argp.add_argument(
        "--error-log", default=f'stock_errors.log',
    )
    argp.add_argument(
        "--success-log", default=f'stock_success.log',
    )
